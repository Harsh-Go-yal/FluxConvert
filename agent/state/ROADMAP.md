...
> ⏸️ Autonomous sessions are **paused** by owner request. Instructions in the pause thread still work.

## Next (ship end-to-end: registry → uploader dispatch → options UI → download)
- [x] Wire `extract-pages` using `lib/pdf/operations.ts#extractPdfPages` + the existing `ui/page-range-selector.tsx`
- [ ] Wire `add-page-numbers` using `lib/pdf/page-numbers.ts` (position + format options panel) — previous attempt made no changes; retry
- [x] Wire `organize-pdf` (drag-to-reorder thumbnails via `lib/pdf/thumbnails.ts` + `reorderPdfPages`)
- [x] Wire `crop-pdf` (set page CropBox with pdf-lib; margin inputs)
...
- [ ] Consolidate duplicate helpers: `lib/pdf-split.ts`, `lib/pdf-watermark.ts`, `lib/pdf-page-numbers.ts`, `lib/pdf-rotate.ts`, `lib/pdf-merge.ts`, `lib/pdf-extract-pages.ts`, `lib/pdf-meta.ts`, `lib/pdf-to-image.ts` → keep one implementation under `lib/pdf/` and delete the orphan (previous attempt made no changes; retry)
